...
async def create_authenticated_client() -> AsyncClient:
    ...
    # Create first user
    await client.post(
        "/users/create-first-user", json={"email": "test@example.com", "password": "password123"}
    )

    # Sign in to get session cookie
    await client.post("/sign-in", json={"email": "test@example.com", "password": "password123"})
...
async def test_create_api_key():
    ...
    try:
        response = await client.post("/api_keys", json={"name": "Test API Key"})
        ...
        assert data["key"].startswith("jtel_")
...
async def test_list_api_keys():
    ...
    try:
        # Create two keys
        response1 = await client.post("/api_keys", json={"name": "Key 1"})
        key1_id = response1.json()["id"]

        response2 = await client.post("/api_keys", json={"name": "Key 2"})
...
async def test_delete_api_key():
    ...
    try:
        # Create a key
        create_response = await client.post("/api_keys", json={"name": "Delete Me"})
...
async def test_api_key_fields_snake_case():
    ...
    client = await create_authenticated_client()
    try:
        response = await client.post("/api_keys", json={"name": "Test Key"})
...
async def test_create_multiple_keys_unique():
    ...
    try:
        ...
        keys = []
        for i in range(10):
            response = await client.post("/api_keys", json={"name": f"Key {i}"})
            assert response.status_code == 201
