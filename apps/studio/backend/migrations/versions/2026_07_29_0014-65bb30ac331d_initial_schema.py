...
def upgrade() -> None:
    ...
    sa.CheckConstraint('length(CAST(prefix AS BLOB)) BETWEEN 1 AND 32', name='evaluation_tokens_prefix_bytes'),
    sa.CheckConstraint('length(secret_hash) = 64', name='evaluation_tokens_secret_hash_length'),
    sa.ForeignKeyConstraint(['created_by_user_id'], ['users.id'], ondelete='SET NULL'),
    sa.PrimaryKeyConstraint('id'),
    sa.UniqueConstraint('secret_hash')
    )
    with op.batch_alter_table('evaluation_tokens', schema=None) as batch_op:
        batch_op.create_index(batch_op.f('ix_evaluation_tokens_prefix'), ['prefix'], unique=True)
...
def downgrade() -> None:
    ...
    with op.batch_alter_table('evaluation_tokens', schema=None) as batch_op:
        batch_op.drop_index(batch_op.f('ix_evaluation_tokens_prefix'))
