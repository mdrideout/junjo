...
class Graph:
    ...
    """
    def __init__(self, source: BaseNode, sink: BaseNode, edges: list[Edge]):
        self.source = source
        self.sink = sink
        self.edges = edges

    def get_next_node(self, current_node: BaseNode) -> BaseNode:
        matching_edges = [edge for edge in self.edges if edge.tail == current_node]
        resolved_edges = [edge for edge in matching_edges if edge.next_node() is not None]

        if len(resolved_edges) == 0:
            raise ValueError(f"No valid transition found for node '{current_node}'.")
        else:
            resolved_edge = resolved_edges[0].next_node()
