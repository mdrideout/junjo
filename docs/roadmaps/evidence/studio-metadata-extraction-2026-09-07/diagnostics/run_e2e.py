...
for rep,order in [(1,['baseline','candidate']),(2,['candidate','baseline'])]:
 for shape in ['mixed','saturation']:
  for v in order:
   label=f'final-e2e-{v}-{shape}-{rep}'
   ...
   cmd=['uv','run','--project','backend','python',str(r/'benchmarks/auth_path_benchmark.py'),'--skip-build','--implementation-label',label,'--exporters','50']
   cmd=cmd+['--exports-per-exporter', '600' if shape=='mixed' else '800','--spans-per-export','32','--query-workers','2' if shape=='mixed' else '0','--export-interval-ms','100' if shape=='mixed' else '0','--skip-revocation','--wal-probe-spans','0','--verify-delivery','--recovery-seconds','0','--output',str(r/'results'/f'{label}.json')]
   ...
   # Preserve failed runs and continue independent comparisons.
