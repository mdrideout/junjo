...
scenarios={
 'mixed':(50,100,32,2,100),
 'sustained':(50,1000,32,0,0),
 'serial':(1,100,512,0,0),
 'sparse':(1,1000,1,0,0),
 'full-batch':(10,100,1000,0,0),
}
for repetition in range(1,4):
    for scenario,(exporters,exports,spans,queries,interval) in scenarios.items():
        ...
        for variant in variants:
            label=f'ingestion-{variant}-{scenario}-r{repetition}'
            image='junjo-durability-baseline:local' if variant=='baseline' else 'junjo-robustness-ingestion:local'
            ...
            args=['uv','run','--project','backend','python','ingestion/benchmarks/auth_path_benchmark.py','--skip-build',
                ...
                '--skip-revocation','--verify-delivery','--wal-probe-spans','0','--output',str(root/'results'/f'{label}.json')]
