...
for repetition in range(1,3):
    for scenario,(exporters,exports,spans,queries,interval) in scenarios.items():
        ...
        for variant in variants:
            label=f'ingestion-drop-{variant}-{scenario}-r{repetition}'
            image='junjo-durability-baseline:local' if variant=='baseline' else 'junjo-robustness-explicit-drop:local'
