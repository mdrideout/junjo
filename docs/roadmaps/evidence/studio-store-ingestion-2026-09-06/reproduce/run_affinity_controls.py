...
variants={'old':'junjo-durability-baseline:local','tests':'junjo-robustness-baseline-exact:local','drop':'junjo-robustness-explicit-drop:local'}
for rep,order in [(1,['old','tests','drop']),(2,['drop','tests','old']),(3,['tests','old','drop'])]:
 for variant in order:
  label=f'affinity-{variant}-sustained-r{rep}'
  env={**os.environ,'JUNJO_BENCHMARK_PROJECT_NAME':'junjo-robustness-benchmark','JUNJO_BENCHMARK_COMPOSE_OVERLAY':str(root/'images-affinity.yaml'),'JUNJO_BENCHMARK_IMAGE':variants[variant]}
