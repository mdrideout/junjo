...
- Added the integer `junjo.telemetry.contract_version` attribute to every Junjo
  executable span so SDK and Studio compatibility is explicit and independent
  of their product versions.

### Tooling and CI

- Moved the Python package, tests, docs, and examples into the independent
  `sdks/python` monorepo workspace without changing the `junjo` package name or
  import path.
